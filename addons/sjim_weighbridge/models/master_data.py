    ...
    def write(self, vals):
        for partner in self:
            if 'referensi_timbang' in vals:
                if not vals.get('referensi_timbang','').strip() and partner.referensi_timbang_id:
                    raise UserError(_('You cannot make edit Referensi Timbang into NULL unless you delete Related Partner in Weighbridge'))
                elif vals.get('referensi_timbang','').strip():
                    if partner.referensi_timbang_id:
                        partner.referensi_timbang_id.write({'name': vals['referensi_timbang'].strip()})
                    else:
                        ref_new = self.env['weighbridge.partner'].create({'name': vals['referensi_timbang'].strip(), 
...
class Product(models.Model):
    ...
    def create(self, vals):
        ...
        if 'referensi_timbang' in vals and vals.get('referensi_timbang','').strip():
    ...
    def write(self, vals):
        for product in self:
            if 'referensi_timbang' in vals:
                if not vals.get('referensi_timbang','').strip() and product.referensi_timbang_id:
                    ...
                    if product.referensi_timbang_id:
                        product.referensi_timbang_id.write({'name': vals['referensi_timbang'].strip()})
                    else:
                        new_ref = self.env['weighbridge.product'].create({'name': vals['referensi_timbang'].strip(), 
...
class PickingType(models.Model):
    ...
    kawasan_berikat = fields.Boolean('Transaksi Kawasan Berikat', default=False)